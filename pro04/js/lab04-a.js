...
window.onload = function initCube() {
    ...
    dicLocx = gl.getUniformLocation(program, "dicx");
    gl.uniform1f(dicLocx, dicx);
    dicLocy = gl.getUniformLocation(program, "dicy");
    gl.uniform1f(dicLocy, dicy);
    dicLocz = gl.getUniformLocation(program, "dicz");
    gl.uniform1f(dicLocz, dicz);
    ...
    document.getElementById("dic").onclick = function () {
        // dic = -dic;
        if(dicflag===xAxis)dicx = -dicx;
        else if(dicflag===yAxis)dicy = -dicy;
        else dicz = -dicz;
        console.log("111"+dic)
    }
...
function makeCube() {
    ...
    for (var i = 0; i < faces.length; i++) {
        ...
        console.log("i="+i+"  "+faces[i][0]+" "+faces[i][1]+" "+faces[i][2]);
...
function render() {
    ...
    setTimeout( function (){ requestAnimFrame( render ); }, 1);
}